...
/**
 * This class was partially from Lab 10 which had some implementations for some
 * of the LList methods. This class provides the structure for a Doubly Linked
 * List.
 *
...
public class DLList<E> {
    ...
    public void sort(Comparator<E> comparator) {
        ...
        boolean madeItThrough;

        do {
            madeItThrough = false;
            Node<E> traversal = head.next();
            for (int x = 0; x < (size - 1); x++) {
                if (comparator.compare(traversal.getData(), (traversal.next())
                    .getData()) > 0) {
                    swap(traversal, traversal.next());
                    madeItThrough = true;
                }
